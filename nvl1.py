...
from classes import Personaje,Items,Background,Animal,Extra,Extra2,Extra3,Boat, Net, muestra_texto, puntuacion, WIN_LOSE
...
def nivel_1():
    ...
    items_health = [25,25,25,25]

    background_animations = []
    background_types = ['river']
    ...
    for boat in boat_types:
        ...
        boat_animations.append(animations_list_7)
    ...
    personaje_1 = Personaje(personaje_animations[0],SCREEN_WIDTH-900,SCREEN_HEIGHT-180,1)#list,x,y,the speed if you move it
    ...
    extra_1 = Extra(extra_animatios[0],40,40,num_of_frames_5)#list,x,y,
    extras_group.add(extra_1)

    extra_2 = Extra2(extra_2_animatios[0],0,0)#list,x,y,
    extras_2_group.add(extra_2)

    boat_1 = Boat(boat_animations[0],100,150)#list,x,y,
    boat_group.add(boat_1)
    ...
    while while_1:
        ...
        if len(items_group) < MAX_ITEMS:
            ...
            if pygame.time.get_ticks() - last_item > ITEMS_TIMER:
                ...
                items = Items(items_health[i],items_animations[i],0,SCREEN_HEIGHT-100,speed) #list,y,x,the speed if you move it
        ...
        if len(animals_group) < MAX_ANIMALS:
            ...
            if pygame.time.get_ticks() - last_animal > ANIMALS_TIMER:
                ...
                animals = Animal(animals_health[j],animals_animations[j],SCREEN_WIDTH,SCREEN_HEIGHT-100,1) #list,y,x,the speed if you move it
                animals_group.add(animals)
                #reset enemy timer 
                last_animal = pygame.time.get_ticks()
                ANIMALS_TIMER = random.randint(5000,10000)
        background_group.update(screen)#draw the background
        boat_group.update(screen)
